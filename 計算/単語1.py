...
print("読み込み中")
model = gensim.models.KeyedVectors.load_word2vec_format('/mnt/chromeos/MyFiles/setup/vector_neologd/model.vec',binary=False)
print("読み込み完了")
...
def generate_and_add_random_int():
    ...
    while i < count:
        ...
        user_answer = custom_keypad_dialog("掛け算問題", question)
        ...
        Question.append([f'{X} × {Y}\n', correct_answer, user_answer, float(execution_time[totalNumber]), judge])
...
print(f'{word}の類似語')
for item ,value in model.most_similar(word, topn=1): # topnを1に設定
    print(item,value)
for i in range(100):
    word = item
    print(f'{word}の類似語')
    for item ,value in model.most_similar(word, topn=2): # topnを3に設定
        print(item,value)